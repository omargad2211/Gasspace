...
const LeftSidebar = () => {
  ...
  return (
    <div className="bg-primary fixed right-0 top-20 md:top-24 hidden md:flex flex-col justify-start items-start gap-4 px-2  h-screen w-1/5">
      ...
      <div className="bg-white h-full md:h-1/2 flex flex-col items-start justify-around px-2 md:px-8 gap-8 md:w-full rounded-lg overflow-y-auto overflow-x-hidden scrollbar-hide pt-3">
        {users
          ?.filter((user) => user.displayName && user.photoURL) // Filter valid users
          .map((user) => (
            <Link
              key={user.id}
              className="flex justify-start items-center gap-2 text-gray-600 hover:bg-gray-600/50 px-2 py-1 w-full rounded-xl hover:text-white"
            >
              <img
                src={user.photoURL}
                alt="profile"
